...
public class Dijkstra {
	...
	public void dijkstra(MAP map) {
		...
		for (int i = 0; i < map.num; i++) { 
			// Initialize the visiting status and the distances
			vis[i] = false;
			dis[i] = 500;
		}
		...
		for (int i = 0; i < map.num; i++) {
			...
			if (u == -1) {
				// If we cannot find any other adjacent vertices, the function is done
				distance = dis[map.end];
				return;
			}
